...
@Api(value = "接口管理,，维护人:psh", tags = {"用户接口管理"})
public class TmUserController {
    ...
    public BaseResultModel getById(@PathVariable("id") Long id) {
        return tmUserService.getOneById(id);
    }
    ...
    public BaseResultModel insert(@RequestBody ReqTmUserAdd req) {
        return tmUserService.insert(req);
    }
    ...
    public BaseResultModel update(@PathVariable("id") Long id, @RequestBody ReqTmUserUpdate req) {
        return tmUserService.update(id, req);
    }
    ...
    public BaseResultModel<IPage<ResTmUser>> list(ReqTmUserQuery req, @PathVariable("pageNum") int pageNum, @PathVariable("pageSize") int pageSize) {
        Page<ResTmUser> page = new Page<>(pageNum, pageSize);
        return tmUserService.page(page, req);
    }
    ...
    public BaseResultModel deleteOne(@PathVariable Long id) {
        return tmUserService.deleteOne(id);
    }
    ...
    public BaseResultModel deleteBatch(@RequestBody List<Long> ids) {
        return tmUserService.deleteBatch(ids);
    }
    ...
    @GetMapping("/getAll")
    public BaseResultModel<List<TmUser>> getAll() {
        QueryWrapper<TmUser> query = new QueryWrapper<>();
        query.eq("deleted", "0");
        return BaseResultModel.success(tmUserService.list(query));
    }
